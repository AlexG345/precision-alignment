...
local function Queue_Add( ply, ent, stackID )
	...
	if !processing then
		processing = true
		local Delay = GetConVar( PA_ .. "stack_delay", 0.1 ):GetFloat()
...
function precision_align_move_func( ply, cmd, args )
	...
	if v == vector_origin then return false end
...
local function rotate_world( ang, rotang )
	...
	if rotang.y ~= 0 then
		ang:RotateAroundAxis( vector_up, rotang.y )
	end
	if rotang.r ~= 0 then
...
function precision_align_constraint_func( len, ply )
	...
	elseif constraint_type == "Wire Hydraulic" then
		local wh_controller = Entity( vars )
		if !IsValid( wh_controller ) then return false end
		if wh_controller:GetClass() ~= "gmod_wire_hydraulic" then
		...
		local oldconstraint = wh_controller.constraint
		local oldrope = wh_controller.rope

		const, rope = MakeWireHydraulic( ply, Ent1, Ent2, 0, 0, LPos1, LPos2, width, material, 0, nil )

		if const then
			wh_controller.MyId = wh_controller:EntIndex()
			const.MyCrtl = wh_controller:EntIndex()
			wh_controller:SetConstraint( const, rope )
			wh_controller:DeleteOnRemove( const )
		end

		if rope then
			wh_controller:DeleteOnRemove( rope )
		end

		-- Remove the existing hydraulic constraint
		if oldconstraint then
			wh_controller:DontDeleteOnRemove( oldconstraint )
			oldconstraint:DontDeleteOnRemove( wh_controller )
			oldconstraint:Remove()
		end

		if oldrope then
			wh_controller:DontDeleteOnRemove( oldrope )
			oldrope:DontDeleteOnRemove( wh_controller )
			oldrope:Remove()
		end
