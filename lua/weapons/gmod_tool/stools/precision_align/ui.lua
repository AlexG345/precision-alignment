...
local point_size_min = math.max( GetConVar( PA_ .. "size_point" ):GetFloat(), 1 )
local point_size_max = GetConVar( PA_ .. "size_point" ):GetFloat() * 1000

local line_size_start = GetConVar( PA_ .. "size_line_start" ):GetFloat()
local line_size_min = GetConVar( PA_ .. "size_line_end" ):GetFloat()	-- End (double bar)
local line_size_max = line_size_min * 1000

local plane_size = GetConVar( PA_ .. "size_plane" ):GetFloat()
local plane_size_normal = GetConVar( PA_ .. "size_plane_normal" ):GetFloat()
...
local function precision_align_draw( w, h)
	...
	if not displayHUD then return end
			...
			if line_temp then
				...
				if IsValid(v.entity) then
					...
					if math.abs(normal.z) < 0.9 then
						dir1 = (normal:Cross(vector_up)):GetNormal()
					else
...
local function precision_align_open_panel_func( ply, cmd, args )
	PA_manipulation_panel = PA_manipulation_panel or vgui.Create( "PA_Manipulation_Frame" )
